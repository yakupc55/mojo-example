...
struct MyNn:
    ...
    fn __copyinit__(inout self, other: Self):
        ...
        self.typeList = other.typeList.copy()
        
    fn addRow(self,size:Int,typeRow:Int):
...
var rowSize:Int = toInt(Float64(size)/colSize);
let irisInput = List[List[Float64]]()
var copyList = List[Float64]()
copyList.addMany(colSize,0.0)
for i in range(rowSize):
    for j in range(colSize):
        copyList[j] = iris.data[i][j].to_float64()
    irisInput.add(copyList.copy())
# ListManager.print2D(myList)
```
...
```mojo
let irisOutput = List[Int]()
var size:Int = iris.target.size.to_index()
irisOutput.addMany(size,0)
for i in range(size):
    irisOutput[i] = iris.target[i].to_index()
# irisOutput.printArray()
```
...
```mojo
fn trainIrisNetwork(nn:MyNn,inputList:List[List[Float64]], outputList:List[Int],trainSize:Int,trainLoop:Int,shakeLoop:Int):
    ...
    for shake in range(shakeLoop):
        ...
        for train in range(trainLoop):
            for s in range(trainSize):
                for t in range(3):
                    let index=(50*t)+s
                    output[0]= outputList[index]
                    nn.forwandAndbackward(inputList[index],output)
```


```mojo
fn testIrisNetworkRate(nn:MyNn,inputList:List[List[Float64]], outputList:List[Int]):
...
```mojo
var irisNetwork:MyNn = MyNn(4,1,0.24)
irisNetwork.finishRows()
```


```mojo
let first= now()
trainIrisNetwork(irisNetwork,irisInput,irisOutput,5,2,1)
...
```
    ...
    10006 ns 0.010005999999999999 ms



```mojo
testIrisNetworkRate(irisNetwork,irisInput,irisOutput)
```

    1.5217092086791659 1
    1.5056678504992584 1
    1.5127515556473361 1
    1.4373291556710526 2
    1.4466701519369831 2
    1.4756796228268649 2
    success rate: 96.0 %
