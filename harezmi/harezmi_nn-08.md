...
struct HNn2:
    ...
    var layerSize:Int
    var squareNorons:List[Noron]
    var squareResultNoron: Noron
    var squareSynapsis: List[Synapsis]
    ...
    fn __init__(inout self,inputSize:Int,outputSize:Int,networkRate:Float64):
        ...
        self.squareSynapsis = List[Synapsis]()
        #functions
        self.addNorons()
    ...
    fn createNoronList(self):
        ...
        #add square Norons
        for i in range(self.squareNorons.size()):
            inputRow.add(self.squareNorons[i])    
        ...
        processRow.add(self.squareResultNoron)
    ...
    fn createSynapsisList(self):
        ...
        #remain synapsis
        self.remainSynapsis.set( Synapsis(self.remainNoron.index.get(),self.remainResultNoron.index.get(),Synapsis_addType,1.0,True,10.0) )
        ...
        #square synapsis
        for i in range(self.squareNorons.size()):
            self.squareSynapsis.add( Synapsis(self.squareNorons[i].index.get(),self.squareResultNoron.index.get(),Synapsis_factorType,1,True,0.1) )
            self.squareResultNoron.synapsisIndexList.add(lastIndex)
            lastIndex = lastIndex + 1
        
        self.synapsisList.add(self.squareSynapsis)
        #outpus synapsis
        ...
        lastIndex = lastIndex + 1
        self.outputSynapsises.add( Synapsis(self.squareResultNoron.index.get(),self.outputNoron.index.get(),Synapsis_factorType,1,False,1.0) )
        self.outputNoron.synapsisIndexList.add(lastIndex)          
    ...
    fn forwardFeed(self, inputList: List[Float64]):
        for i in range(self.inputSize.get()):
            ...
            self.squareNorons[i].data.set( inputList[i] * inputList[i] )
    ...
    fn giveTheFormula(self):
        ...
        #square
        let x2:Float64= roundBigF(self.squareSynapsis[0].value.get())
        process = String(x2)+"x²"
        for i in range(1,self.inputSize.get()):
            let num:Float64= roundBigF(self.squareSynapsis[i].value.get())
            if num>=0:
                process += String("+")
...
for i in range(inputList.size()):
    ...
    let calculate =(num1*num1*2)+(num2*num2)+(num1*-3)+(num2*3)+20
...
```
    ...
    [110,2353,1102,957,3142,1082,2270,688,1532,538,1604,2738,]



```mojo
#craete networl
var normalNetwork:HNn2 = HNn2(2,1,0.011)
...
print(result,"ns",Float16(result/1000000.0),"ms")
...
```
    ...
    f(x,y) : 2.0x²+1.0y²-3.0x+3.0y+20.0
